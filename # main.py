import sqlite3
from personnage import Personnage, sauvegarder_personnages, charger_personnages
from equipe import selectionner_equipe
from combat import boucle_combat
from adversaire import Adversaire, ajouter_adversaires, sauvegarder_adversaires
from equipe_adverse import EquipeAdverse
from bddmanager import creer_base_de_donnees, sauvegarder_objets, charger_donnees
...
def charger_donnees(table, objet_classe):
    with DatabaseManager() as db:
        ...
        return [objet_classe(*row) for row in db.fetchall()]
...
def inserer_donnees_de_test():
    ...
    personnages_test = [
        ("Merlin cliché", 50, 50, 100, 100, 20, 1, 1, 0, 0),
        ("Artxis le troll de Skyrim avec la montagne des Grisebarbes", 200, 200, 150, 150, 5, 1, 1, 0, 0),
        ("Pedro", 225000, 225000, 20000, 20000, 20000, 1, 1, 0, 0),
        ("Conan", 100, 100, 100, 100, 10, 1, 1, 0, 0)
    ]
    ...
    adversaires_test = [
        ("Adversaire 1", 100, 100, 50, 50, 10, 1),
        ("Adversaire 2", 150, 150, 75, 75, 15, 1)
    ]
...
def tester_jeu():
    ...
    if not personnages:
        merlin = Personnage(1, "Merlin cliché", 50, 50, 100, 100, 20, 1, 1, 0, 0)
        artxis = Personnage(2, "Artxis le troll de Skyrim avec la montagne des Grisebarbes", 200, 200, 150, 150, 5, 1, 1, 0, 0)
        pedro_le_singe = Personnage(3, "Pedro", 225000, 225000, 20000, 20000, 20000, 1, 1, 0, 0)
        conan_guerrier = Personnage(4, "Conan", 100, 100, 100, 100, 10, 1, 1, 0, 0)
